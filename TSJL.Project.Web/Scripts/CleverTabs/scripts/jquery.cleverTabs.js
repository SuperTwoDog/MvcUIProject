...
CleverTabs.prototype.getCurrentUniqueId = function () {
    var self = this;
    if (self.element.find(' > li').length > 0) {
        var current = self.element.find('li.ui-tabs-selected');
        return current.length > 0 ? CleverTab.getUniqueByHeaderId(current.attr('id')) : null;
    } else {
        return null;
    }
}
...
CleverTabs.prototype.clear = function () {
    ...
    var hasLock = self.element.find('span.ui-icon-locked').length > 0;
    for (i = self.lockOnlyOne && !hasLock ? 1 : 0; i < lis.length; i++) {
...
function CleverTab(tabs, id) {
    ...
    //Tab页面是否锁定(不允许关闭)
    this.lock = this.header.find('span.ui-icon-locked').length != 0;
    //Tab页面有于显示内容的面板
...
CleverTab.prototype.deactivate = function () {
    ...
    if (self.tabs.lockOnlyOne && !self.tabs.hashtable[self.id].orgLock && self.tabs.element.find('>li').length > 1) {
        self.setLock(false);
    }
}
//使Tab页面处于激活状态
CleverTab.prototype.activate = function () {
    ...
    if (self.tabs.lockOnlyOne && self.tabs.element.find('>li').length == 1) {
...
CleverTab.prototype.setLock = function (lock, changeOrgLock) {
    ...
    if (!lock && self.tabs.lockOnlyOne && self.tabs.element.find('>li').length == 1) {
        return;
    }
    if (!lock) {
